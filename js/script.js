...
// Function to render the team list based on a filtered array
const renderList = (filteredTeam) => {
...
const search = () => {
  ...
  const filteredTeam = team.filter(member => member.name.toLowerCase().includes(searchTerm));
  
  // Check if there's a search term; if not, render the full list again
  if (searchTerm === '') {
    renderList(team);
  } else {
    // If a driver is found, render only the matching driver
    renderList(filteredTeam);
  }
};
